...
public:
	...
	//Topo
	bool printTopo() { //directed graphs only
		if (!directed) {
...
int main() {
	...
	Edge<int>** edgetmp=new Edge<int>*[n];
	for (i = 0; i < n; i++) {
		edgetmp[i] = new Edge<int>;
	}
	...
	void caculate(int **matrix,int numVer, int n);
	AMGraph<int> test(arr, m,  edgetmp, n, true);
	if (test.printTopo())
...
void caculate(int **matrix, int numVer, int n) {
	...
	while (!q.IsEmpty()) {
		...
		for (i = 0; i < numVer; i++) {
			if (matrix[tmp][i] > 0) {
				maxdis[i] = maxdis[i] > maxdis[tmp] + matrix[tmp][i] ? maxdis[i] : maxdis[tmp] + matrix[tmp][i];
				q.EnQueue(i);
			}
		}
	}
	...
	for (i = 0; i < numVer; i++) {
		cout << left << setw(6) << i+1 << maxdis[i]<<endl;
	}
}